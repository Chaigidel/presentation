...
def main():
    ...
    result = [proc.get() for proc in procs]
        # print('F')
    json.dump(result, open('task4_result.json', 'w'))

def draw():
    tests = load_json('task2.json')
    datas = load_json('task4_result.json')
    LAYERS = tests
    ...
    ax.set_title('不同层数（每层节点 50 个）对识别率的影响', fontproperties=zhfont1)
